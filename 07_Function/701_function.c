...
// 함수 이름 myFunc 으로 함수 정의 function definition
// 리턴 타입 : void
// 매개변수 : () 없음
void myFunc()
{
...
// 이름 : oper
// 매개변수 : int, int
// 리턴타입 : void (리턴 값이 없는 경우, 반드시 void 명시)
// 동작 : 두 개의 정수값을 받아서 사칙연산 결과 출력
...
int main()
{
	...
	// 코드가 DRY 해요.. (Don't Repeat Yourself) - 코드가 반복되는 것이 많다는 얘기.

	printf("----------------------------\n");
	{
		printf("[main() : myFunc() 호출 전]\n");
		myFunc(); // main() 이 myFun() 함수 호출
		printf("[main() : myFunc() 리턴 후]\n");
		
		oper(40, 50);  // oper() 함수 호출, 매개변수 k, v에 40, 50 이 전달 (복사 발생)
		oper(10, 20);
		oper(100, 200);
		oper(-300, 100);
	}
